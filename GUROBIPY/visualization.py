...
class Visualization_TOPF:
    ...
    def save_plot_topf_milp(self, model, name, auto_open_flag=0):
        ...
        with open(name + '.csv', 'w') as csv_file:
            ...
            writer.writerow([len(self.W), len(self.T), len(self.D), self.L, self.T_max])
...
class Visualization_TOPTW:
    def __init__(self, model, W, T, D, S, E, S_loc, E_loc, T_loc, c, A, Q, R, T_max, arrival):
        self.W = W
        self.T = T
        self.D = D
        self.S = S
        ...
        for i in range(len(self.v)):
            ...
            # if self.v[i].x >= 0.9 and self.v[i].varName[0] == 's':
            #     print(self.v[i].varName, self.v[i].x)
            #     w, n, time = self.v[i].VarName.split(']')
            #     w = w.replace("s[", "")
            #     w = w.replace("]", "")
            #     print(w, time)
            #     self.s[w] = time
        ...
        for w in self.W:
            for arc in self.arcsInOrder[w]:
                if arc[1] != 'E':
                    # self.arrivalTimes[w,arc[1]].append(s[w, arc[1]].x)
    ...
    def save_plot_toptw(self, model, name, auto_open_flag=0):
        ...
        with open(name + '.csv', 'w') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow([len(self.W), len(self.T), self.T_max])
            for key, value in self.arcsInOrder.items():
                writer.writerow([key, value])
